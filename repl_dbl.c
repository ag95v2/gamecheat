...
t_list	*scan_heap(t_memmap *memmap, pid_t pid, double value)
{
	...
	while (memmap )
	{
	 //memmap->info->type == shlibs || ENABLE search in dinamic libs
		if ( memmap->info->type == other || memmap->info->type == driver_maps )
		{
			memmap = memmap->next;
			continue ;
		}
...
{
	int	h, st, bss, anon, oth, shl;
	enum region_type t;	

	shl = oth = h = st = bss= anon = 0;
	while (addrs)
	{
